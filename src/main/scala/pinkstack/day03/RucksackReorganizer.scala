...
object RucksackReorganizer:
  private val priorities = (('a' to 'z') ++ ('A' to 'Z')).zipWithIndex.map((c, i) => c -> (i + 1)).toMap

  private def duplicates[B](splitWith: Chunk[String] => B, intersection: B => Char, chunkSize: Int = 1)(
    path: Path
  ): Task[Int] =
    streamFromPath(path)
